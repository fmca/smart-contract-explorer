...
contract SimulationExamples {
    ...
    function getA(uint index) private view returns (uint) {
        (uint a, ) = spec.containers(index);
        return a;
    }

    function getB(uint index) private view returns (uint) {
        (,uint b) = spec.containers(index);
        return b;
    }
